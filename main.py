...
def main():
    ...
    duplicate_entries = preprocessed_data[preprocessed_data.duplicated(subset=['Age', 'Clothing ID'], keep=False)]
    ...
    # Remove duplicates
    preprocessed_data = preprocessed_data.drop_duplicates(subset=['Age', 'Clothing ID'])

    # Pivot data to create user-item matrix
    user_item_matrix = preprocessed_data.pivot(index='Age', columns='Clothing ID', values='Rating')
